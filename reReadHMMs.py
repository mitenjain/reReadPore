...
def event_hash(filePath):
    class_event_hash = {}
    coordsFile = open(os.path.join(filePath, 'coords.txt'))
    ...
    for stanza in getStanza(coordsFile):
        ...
        filenameKey = filename.strip().split('_')[0]
...
#                                                              'robin_500mer.txt'))
    ...
    '''
    Construct models: reRead_500
    '''
    reRead_500_model = model_maker( kmer_current_dict_adapter_bg, \
                                                       model_name = 'reRead_500' )
    models = [ reRead_500_model ]
...
#     models[0].write(sys.stdout)
    ...
    viterbi_prediction = []
    # printing template
    column_output_template = '{0:>} {1:>5} {2:>5} {3:>5}'
    data_output_template = '{0:>d} {1:>5d} {2:>5d} {3:>5d}'
...
#                 if filenamekey == '16831001-s04.abf':
#                     print count, '\t', segment.mean, '\t', segment.std
            ...
            print filename, event.start, event.end, scores
            num_events += 1
...
#     print >> sys.stdout, column_output_template.format('T5', 'T6', 'T7', 'T8')
#     print >> sys.stdout, data_output_template.format(t5, t6, t7, t8) 
